...
void writeActivePc(std::vector<std::shared_ptr<dsm::Frame>> & frames,
                   std::string & fname) {
...
void read_kitti_pose_file(const std::string & tracking_fname,
  ...
  while (std::getline(gt_file, line)) {
    ...
    Eigen::Vector3f trans = pose_c.block<3,1>(0,3); 
    Eigen::Matrix3f rot = pose_c.block<3,3>(0,0);
    Sophus::SE3f pose_sophus(rot, trans);
    std::cout<<"Read pose \n"<<pose_c<<std::endl;
...
void static_depth_stereo(std::vector<int> frame_inds,
  ...
  for (int i = 0; i < frame_inds.size(); i++) {
    ...
    double timestamp = 0;
    std::shared_ptr<dsm::Frame> trackingNewFrame = std::make_shared<dsm::Frame>(id, timestamp, gray_img.data, source_raw, source_pcd,
                                                                      source_full);
  ...
  std::string fname = "before_BA.pcd";
  //frames.pop_back();
  writeActivePc(frames, fname);
...
int main(int argc, char *argv[])
{
  ...
  read_kitti_pose_file(trackingFname, frame_inds, poses);
  ...
  static_depth_stereo(frame_inds, poses, kitti,  settingsFile,
                      calib, cvoConfigFile);
