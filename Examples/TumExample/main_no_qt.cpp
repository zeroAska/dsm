...
namespace dsm
{
  class TumProcessorNoQT
  {
  public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
    inline TumProcessorNoQT() { this->shouldStop = false; }
    inline ~TumProcessorNoQT() { this->join(); }

    inline void run(cvo::TumHandler& reader, std::string& settingsFile,
                    std::string& cvoConfigFile, cvo::Calibration& cvo_calib, int startFrameId,
                    std::string& trajFileName
                    )
    {
      this->processThread = std::make_unique<std::thread>(&TumProcessorNoQT::doRun, this,
                                                          std::ref(reader),
...
int main(int argc, char *argv[])
{
  ...
  dsm::TumProcessorNoQT processor;
  processor.run(tum,  settingsFile, cvoConfigFile, calib, startFrameId, trajFileName);
