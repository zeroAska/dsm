...
namespace dsm
{
  ...
  void MonoInitializer::setReference(const std::shared_ptr<Frame>& firstFrame)
  {
    ...
    //this->minNumTracked = static_cast<int>(0.4f*candidates.size());
    ...
    cv::imwrite("refImage.png", this->refImage);
  ...
  bool MonoInitializer::initialize(const std::shared_ptr<Frame>& frame, Sophus::SE3f& pose)
  {
    ...
    {
      std::cout << "MonoInitializer::initialize not enough tracked points "<< this->refPoints.size()<< ", min n! RESET\n";
    ...
    cv::imwrite("initMotion.png", motion);
